...
generate:
	...
	@{ \
		...
		for f in $(GEN_DIR)/shared.ts $(GEN_DIR)/exercise.ts $(GEN_DIR)/equipment.ts; do \
			tail -n +2 "$$f"; \
		done; \
	...
	@rm -f $(GEN_DIR)/shared.ts $(GEN_DIR)/exercise.ts $(GEN_DIR)/equipment.ts $(GEN_DIR)/user-workout.ts $(GEN_DIR)/user-workoutlog.ts $(GEN_DIR)/user-exerciselog.ts
