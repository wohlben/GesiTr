...
func TestListWorkoutSectionItems(t *testing.T) {
	...
	t.Run("empty list", func(t *testing.T) {
		w := doJSON(r, "GET", "/api/user/workout-section-items", nil)
	...
	t.Run("filter by workoutSectionId", func(t *testing.T) {
		w := doJSON(r, "GET", "/api/user/workout-section-items?workoutSectionId=1", nil)
		var result []models.WorkoutSectionItem
		json.Unmarshal(w.Body.Bytes(), &result)
		if len(result) != 2 {
			t.Errorf("expected 2, got %d", len(result))
	...
	t.Run("filter by nonexistent section", func(t *testing.T) {
		w := doJSON(r, "GET", "/api/user/workout-section-items?workoutSectionId=999", nil)
		var result []models.WorkoutSectionItem
		json.Unmarshal(w.Body.Bytes(), &result)
		if len(result) != 0 {
			t.Errorf("expected 0, got %d", len(result))
...
func TestCreateWorkoutSectionItem(t *testing.T) {
	...
	t.Run("bad json", func(t *testing.T) {
		w := doRaw(r, "POST", "/api/user/workout-section-items", "{invalid")
