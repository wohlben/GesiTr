...
func TestFullWorkoutToLogFlow(t *testing.T) {
	...
	var scheme models.UserExerciseScheme
	...
	w = doJSON(r, "POST", "/api/user/workout-sections", map[string]any{
		"workoutId": workout.ID, "type": "supplementary", "label": "Warmup", "position": 0,
	})
	if w.Code != http.StatusCreated {
	...
	w = doJSON(r, "POST", "/api/user/workout-sections", map[string]any{
		"workoutId": workout.ID, "type": "main", "position": 1, "restBetweenExercises": 120,
	})
	if w.Code != http.StatusCreated {
		t.Fatalf("create main section: status = %d", w.Code)
	}
	var mainSection models.WorkoutSection
	...
	w = doJSON(r, "GET", "/api/user/workouts/"+itoa(workout.ID), nil)
	if w.Code != http.StatusOK {
		t.Fatalf("get workout: status = %d", w.Code)
	}
	json.Unmarshal(w.Body.Bytes(), &workout)
	if len(workout.Sections) != 2 {
		t.Fatalf("expected 2 sections, got %d", len(workout.Sections))
	}
	if len(workout.Sections[1].Exercises) != 2 {
		t.Fatalf("expected 2 exercises in main section, got %d", len(workout.Sections[1].Exercises))
	}
	if workout.Sections[1].RestBetweenExercises == nil || *workout.Sections[1].RestBetweenExercises != 120 {
	...
	if fullLog.WorkoutID == nil || *fullLog.WorkoutID != workout.ID {
	...
	w = doJSON(r, "GET", "/api/user/workout-logs?workoutId="+itoa(workout.ID), nil)
	var filteredLogs []models.WorkoutLog
	json.Unmarshal(w.Body.Bytes(), &filteredLogs)
	if len(filteredLogs) != 1 || *filteredLogs[0].WorkoutID != workout.ID {
		t.Errorf("workoutId filter: expected 1 log linked to workout %d, got %d", workout.ID, len(filteredLogs))
	}
