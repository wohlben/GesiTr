...
func ListExercises(c *gin.Context) {
	db := database.DB.Model(&ExerciseEntity{})
	...
	dtos := make([]Exercise, len(entities))
...
func CreateExercise(c *gin.Context) {
	...
	database.DB.Create(&ExerciseHistoryEntity{
...
func UpdateExercise(c *gin.Context) {
	...
	err := database.DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Where("exercise_id = ?", entity.ID).Delete(&ExerciseForce{}).Error; err != nil {
			return err
		}
		if err := tx.Where("exercise_id = ?", entity.ID).Delete(&ExerciseMuscle{}).Error; err != nil {
			return err
		}
		if err := tx.Where("exercise_id = ?", entity.ID).Delete(&ExerciseMeasurementParadigm{}).Error; err != nil {
			return err
		}
		if err := tx.Where("exercise_id = ?", entity.ID).Delete(&ExerciseInstruction{}).Error; err != nil {
			return err
		}
		if err := tx.Where("exercise_id = ?", entity.ID).Delete(&ExerciseImage{}).Error; err != nil {
			return err
		}
		if err := tx.Where("exercise_id = ?", entity.ID).Delete(&ExerciseAlternativeName{}).Error; err != nil {
			return err
		}
		if err := tx.Where("exercise_id = ?", entity.ID).Delete(&ExerciseEquipment{}).Error; err != nil {
			return err
		}
		...
		resultDTO := entity.ToDTO()
		if err := tx.Create(&ExerciseHistoryEntity{
			ExerciseID: entity.ID,
			Version:    resultDTO.Version,
			Snapshot:   shared.SnapshotJSON(resultDTO),
			ChangedAt:  time.Now(),
...
func DeleteExercise(c *gin.Context) {
	if err := database.DB.Delete(&ExerciseEntity{}, c.Param("id")).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "Exercise not found"})
		return
	}
