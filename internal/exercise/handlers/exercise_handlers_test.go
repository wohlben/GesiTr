...
func TestListExercises(t *testing.T) {
	...
	t.Run("owner=me returns only own exercises", func(t *testing.T) {
		w := doJSON(r, "GET", "/api/exercises?owner=me", nil)
		if w.Code != http.StatusOK {
			t.Fatalf("status = %d", w.Code)
		}
		var page paginatedJSON
		...
		json.Unmarshal(page.Items, &result)
		if len(result) != 2 {
			t.Errorf("expected 2 own exercises, got %d", len(result))
		}
	})
	...
	t.Run("public exercises visible to other users", func(t *testing.T) {
		...
		// As bob, list all exercises — should see the public one
		w := doJSONAs(r, "GET", "/api/exercises", nil, "bob")
		if w.Code != http.StatusOK {
			t.Fatalf("status = %d", w.Code)
		}
		var page paginatedJSON
	...
	t.Run("private exercises hidden from other users", func(t *testing.T) {
		// As bob, list with owner=testuser — should only see public exercises from testuser
		w := doJSONAs(r, "GET", "/api/exercises?owner=testuser", nil, "bob")
		if w.Code != http.StatusOK {
			t.Fatalf("status = %d", w.Code)
		}
		var page paginatedJSON
	...
	t.Run("filter public=true", func(t *testing.T) {
		w := doJSON(r, "GET", "/api/exercises?public=true", nil)
		if w.Code != http.StatusOK {
			t.Fatalf("status = %d", w.Code)
		}
		var page paginatedJSON
...
func TestDeleteExercise(t *testing.T) {
	...
	t.Run("not found", func(t *testing.T) {
		w := doJSON(r, "DELETE", "/api/exercises/999", nil)
		if w.Code != http.StatusNotFound {
			t.Errorf("expected 404, got %d", w.Code)
		}
	})
