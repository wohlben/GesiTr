import { Component, inject, computed } from '@angular/core';
import { ActivatedRoute, RouterLink } from '@angular/router';
import { DatePipe, JsonPipe } from '@angular/common';
import { toSignal } from '@angular/core/rxjs-interop';
import { injectQuery } from '@tanstack/angular-query-experimental';
import { CompendiumApiClient } from '$core/api-clients/compendium-api-client';
import { equipmentKeys } from '$core/query-keys';
import { PageLayout } from '../../../layout/page-layout';

@Component({
  selector: 'app-equipment-history',
  imports: [PageLayout, RouterLink, DatePipe, JsonPipe],
  template: `
    <app-page-layout
      [header]="(equipmentQuery.data()?.displayName ?? 'Equipment') + ' — History'"
      [isPending]="equipmentQuery.isPending() || versionsQuery.isPending()"
      [errorMessage]="
        equipmentQuery.isError()
          ? equipmentQuery.error().message
          : versionsQuery.isError()
            ? versionsQuery.error().message
            : undefined
      "
    >
      <a
        actions
        routerLink=".."
        class="rounded-md bg-gray-600 px-4 py-2 text-sm font-medium text-white hover:bg-gray-700"
        >Back</a
      >
      @if (versionsQuery.data(); as versions) {
        <div class="space-y-4">
          @for (entry of versions; track entry.version) {
            <div
              class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800"
            >
              <div class="mb-2 flex items-center justify-between">
                <span class="text-sm font-semibold text-gray-900 dark:text-gray-100"
                  >Version {{ entry.version }}</span
                >
                <span class="text-xs text-gray-500 dark:text-gray-400">
                  {{ entry.changedAt | date: 'medium' }} by {{ entry.changedBy }}
                </span>
              </div>
              <pre
                class="overflow-x-auto rounded bg-gray-50 p-2 text-xs text-gray-700 dark:bg-gray-900 dark:text-gray-300"
                >{{ entry.snapshot | json }}</pre
              >
            </div>
          }
        </div>
      }
    </app-page-layout>
  `,
})
export class EquipmentHistory {
  private api = inject(CompendiumApiClient);
  private params = toSignal(inject(ActivatedRoute).paramMap);

  private id = computed(() => Number(this.params()?.get('id')));

  equipmentQuery = injectQuery(() => ({
    queryKey: equipmentKeys.detail(this.id()),
    queryFn: () => this.api.fetchEquipmentItem(this.id()),
    enabled: !!this.id(),
  }));

  versionsQuery = injectQuery(() => ({
    queryKey: equipmentKeys.versions(this.id()),
    queryFn: () => this.api.fetchEquipmentVersions(this.id()),
    enabled: !!this.id(),
  }));
}
