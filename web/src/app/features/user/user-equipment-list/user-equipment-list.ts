...
@Component({
  ...
  template: `
    <app-page-layout
      ...
      [errorMessage]="userEquipmentQuery.isError() ? userEquipmentQuery.error().message : undefined"
    >
      @if (enrichedEquipment(); as equipment) {
        @if (equipment.length === 0) {
          <p class="text-sm text-gray-500 dark:text-gray-400">No equipment imported yet.</p>
        } @else {
